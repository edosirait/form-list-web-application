...
export class AccountService {
  ...
  constructor(
    ...
    this.employeeSubject = new BehaviorSubject<Employee>(JSON.parse(localStorage.getItem('registration-employee')));
  ...
  updateEmployee(id, params) {
    ...
    console.log('ini employeeValue', this.employeeValue.id);
    return this.http.put(`${environment.apiUrl}/employee/${id}`, params)
      .pipe(map(x => {
        console.log('ini employeeValue', this.employeeValue.id);
        // update stored user if the logged in employees updated their own record
        if (id === this.employeeValue.id) {
